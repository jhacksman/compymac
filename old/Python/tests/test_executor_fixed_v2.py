...
class MockLLM(BaseLLM, Runnable):
    ...
    async def ainvoke(self, input_str: str, config=None) -> str:
        """Mock async invoke method."""
        return self.invoke(input_str, config)
        
    def _generate(self, prompts: List[str], stop: Optional[List[str]] = None, run_manager=None, **kwargs) -> LLMResult:
        """Mock generate method."""
        return LLMResult(generations=[[Generation(text=self.invoke(prompts[0]))]])
...
def test_verify_success(executor_agent):
    ...
    assert executor_agent._verify_success(result, criteria)
    ...
    assert not executor_agent._verify_success(result, criteria)
    
    # Test empty criteria
    assert executor_agent._verify_success(result, {})
    
    # Test invalid result format
    assert not executor_agent._verify_success(None, criteria)
...
def test_calculate_delay(executor_agent):
    ...
    executor_agent.config = config
    
    # Test initial delay
    delay = executor_agent._calculate_delay(0)
    assert 0.9 <= delay <= 1.1  # Account for jitter
    
    # Test exponential increase
    delay = executor_agent._calculate_delay(1)
    assert 1.8 <= delay <= 2.2
    
    # Test maximum delay
    delay = executor_agent._calculate_delay(10)
